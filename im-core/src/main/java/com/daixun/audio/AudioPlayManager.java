    ...
    private void setScreenOff() {
        if (this._wakeLock == null) {
            if (Build.VERSION.SDK_INT >= 21) {
                this._wakeLock = this._powerManager.newWakeLock(32, "im_record:AudioPlayManager");
            } else {
...
public class AudioPlayManager implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        float range = event.values[0];
        boolean isPlaying = this.mediaPlayer != null ? mediaPlayer.isPlaying() : false;
        Log.d(TAG, "onSensorChanged range is:" + range + " isPlaying:" + isPlaying);
        if (this.sensor != null && this.mediaPlayer != null) {
            if (this.mediaPlayer.isPlaying()) {
                if ((double) range > 0.0D) {
                    if (this.audioManager.getMode() == MODE_NORMAL && audioManager.isSpeakerphoneOn()) {
                        return;
                    }

                    this.audioManager.setMode(MODE_NORMAL);
                    this.audioManager.setSpeakerphoneOn(true);
                    final int positions = this.mediaPlayer.getCurrentPosition();

                    try {
                        this.mediaPlayer.reset();
                        this.mediaPlayer.setAudioStreamType(AudioManager.MODE_IN_COMMUNICATION);
                    ...
                    this.setScreenOff();
                    if (Build.VERSION.SDK_INT >= 11) {
                        if (this.audioManager.getMode() == AudioManager.MODE_IN_COMMUNICATION) {
                            return;
                        }

                        this.audioManager.setMode(AudioManager.MODE_IN_COMMUNICATION);
                    } else {
                        if (this.audioManager.getMode() == MODE_IN_CALL) {
                            return;
                        }

                        this.audioManager.setMode(MODE_IN_CALL);
                    }
                ...
                if (this.audioManager.getMode() == MODE_NORMAL) {
                    return;
                }
    ...
    private void setScreenOff() {
        if (this.wakeLock == null) {
            if (Build.VERSION.SDK_INT >= 21) {
                this.wakeLock = this.powerManager.newWakeLock(32, "im_audio:AudioPlayManager");
            } else {
                Log.e(TAG, "Does not support on level " + Build.VERSION.SDK_INT);
            }
        }
